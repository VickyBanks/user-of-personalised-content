...
CREATE TABLE watched_journeys_1 AS
    SELECT app_type,
           ...
           substring(content_id_sequence, pos, length(content_id_sequence))                             AS content_id_after, -- Get content IDs from the after the search page
...
SELECT count(visit_id) FROM watched_journeys_1;
SELECT count(visit_id) FROM watched_journeys_2;
SELECT count(visit_id) FROM watched_journeys_3;
SELECT count(visit_id) FROM watched_journeys_4;
SELECT count(visit_id) FROM watched_journeys_5;


-- journeys after watching_page|episode_page first ex
WITH vb_after_first AS (
    -- Get the first example of watching_page|episode_page in the visit. Find the page number and the id of the content.
    SELECT app_type,
           dt,
           visit_id,
           hashed_id,
           page_sequence,
           CHARINDEX('watching_page|episode_page', page_sequence)                                    AS index,                 --position of 'watching_page' within the journey (position is of the 's')
           RIGHT(page_sequence, length(page_sequence) - index + 1)                                   AS journey_after,         --get everything from this position onwards
           length(LEFT(page_sequence, index)) -
           length(replace(LEFT(page_sequence, index), '|', ''))                                      AS page_num_within_visit, -- what number page was 'search_page' within the visit?
           REGEXP_INSTR(content_id_sequence, '\\|', 1, page_num_within_visit) + 1                    AS pos,-- find the position of this page within the content_id string
           substring(content_id_sequence, pos, length(content_id_sequence))                          AS content_id_after       -- Get content IDs from the after the search page
    FROM central_insights_sandbox.dataforce_journey_complete
    WHERE page_sequence LIKE '%watching_page|episode_page%'
      AND app_type != 'mobile-app'
     ...
     watch_ep_flags AS (
         SELECT a.dt,
                ...
                a.hashed_id,
                a.app_type,
                a.page_num_within_visit + 2                                     as content_page_num,
                b.page_count,
                split_part(a.content_id_after, '|', 2)                          AS content_id,
                b.content_id,
